            ...
            if (Input.acceleration != Vector3.zero)
            {
                switch (tiltAroundAxis)
                {
                    case AxisOptions.ForwardAxis:
                        //angle = Mathf.Atan2(Input.acceleration.x, -Input.acceleration.y)*Mathf.Rad2Deg + centreAngleOffset;
					angle = Input.acceleration.x*Mathf.Rad2Deg*centreAngleOffset;
					break;
                    case AxisOptions.SidewaysAxis:
                        //angle = Mathf.Atan2(Input.acceleration.z, -Input.acceleration.y)*Mathf.Rad2Deg + centreAngleOffset;
						angle = -Input.acceleration.y*Mathf.Rad2Deg*centreAngleOffset;
					break;
                }
            }
			//Debug.Log ("Acceleration:\nX: " + (Input.acceleration.x*Mathf.Rad2Deg) + "   Y: " + (Input.acceleration.y*Mathf.Rad2Deg) + "  Z: "+(Input.acceleration.z*Mathf.Rad2Deg));
